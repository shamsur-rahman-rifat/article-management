...
export const viewArticleList = async (req, res) => {
  try {
    const articles = await articleModel.find()
      .populate('topic', 'title month type writer') // Populate topic with selected fields
      .populate('project', 'name'); // Populate project with name
...
export const updateArticle = async (req, res) => {
  try {
    ...
    const isPublisher = user.roles.includes("publisher") && (!article.publisher || article.publisher === userEmail);
    ...
    // Publisher or admin can update publish link
    if ((isPublisher || isAdmin) && updateData.publishLink) {
